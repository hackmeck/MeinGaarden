...
import com.example.meingaarden.MeinGaardenAppWidget;
...
import com.example.meingaarden.ui.DetailActivity;
import com.example.meingaarden.ui.DetailHotSpotActivity;
...
import static com.example.meingaarden.provider.TouristAttractions.ATTRACTIONS;
...
public class UtilityService extends IntentService {
    ...
    private static final String ACTION_ADD_HOTSPOTGEOFENCES = "add_hotspotgeofences"; // neu von flo
    ...
    /** testweise deaktiviert, da nicht verwendet ?
    ...
    // neu von flo - wird in ui.HotSpotsListActivity.java Zeile 142 verwendet
    public static void addHotSpotGeofences(Context context) {
        Intent intent = new Intent(context, UtilityService.class);
        intent.setAction(UtilityService.ACTION_ADD_HOTSPOTGEOFENCES); // ACTION_ADD_HOTSPOTGEOFENCES in Zeile 82 definiert
        context.startService(intent);
    }
    ...
    */
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        //} else if (ACTION_ADD_HOTSPOTGEOFENCES.equals(action)) {
        //        addHotSpotGeofencesInternal();
     ...
     * Add geofences using Play Services
     */
    private void addHotSpotGeofencesInternal() { // neu
        Log.v(TAG, ACTION_ADD_HOTSPOTGEOFENCES); // neu
        ...
        if (connectionResult.isSuccess() && googleApiClient.isConnected()) {
            PendingIntent pendingIntent = PendingIntent.getBroadcast(
                    this, 0, new Intent(this, UtilityReceiver.class), 0);
            GeofencingApi.addGeofences(googleApiClient,
                    HotSpots.getGeofenceList(), pendingIntent); // neu! kommt aus provider.HotSpots.java
            googleApiClient.disconnect();
        } else {
            Log.e(TAG, String.format(Constants.GOOGLE_API_CLIENT_ERROR_MSG,
                    connectionResult.getErrorCode()));
    ...
    private void geofenceTriggered(Intent intent) {
        ...
        if (geofenceEnabled && geofences != null && geofences.size() > 0) {
            if (event.getGeofenceTransition() == Geofence.GEOFENCE_TRANSITION_ENTER) {
                ...
                SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(UtilityService.this); // neu von flo
                ...
                if (sharedPref.getBoolean("notifications_new_hotspotmessage", false)) {
                    showHotSpotNotification(geofences.get(0).getRequestId(), Constants.USE_MICRO_APP);
                }

                //sendDataToWidget(geofences.get(0).getRequestId());
                return;
    ...
    private void showNotification(String cityId, boolean microApp) {
        ...
        /** vorerst auskommentiert, da ich keine microApp habe
        if (microApp) {
        ...
        */
        ...
        //Uri soundUri = RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION); // alt funktioniert, verwendet standarton aus system-settings
        // neu von flo um an sharedpreferences ringtone ranzukommen
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this)
                ...
                .setSound(soundUri) // von flo um einen sound zu erhalten
                .setContentIntent(pendingIntent)
                .setDeleteIntent(deletePendingIntent)
                //.setColor(getResources().getColor(R.color.colorPrimary, getTheme()))
                //.setCategory(Notification.CATEGORY_RECOMMENDATION) //setzt API 21 voraus
                .setAutoCancel(true);
                // Vibrieren, wenn dies in Settings aktiviert
                SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(UtilityService.this); // neu von flo
                if (sharedPref.getBoolean("notifications_new_message_vibrate", false)) {
                    builder.setDefaults(Notification.DEFAULT_VIBRATE); // Vibrieren aus Systemeinstellungen
                    //builder.setVibrate(new long[] {1000, 1000, 1000, 1000, 1000}) // eigenes Vibrieren
                }
        /** test uakommentiert, da eigentlich keine wearables unterstützt werden
        if (!microApp) {
     ...
     * @param microApp If the micro app should be triggered or just enhanced notifications
     */
    private void showHotSpotNotification(String cityId, boolean microApp) {
        ...
        try {
            for (int i = 0; i < count; i++) {
                bitmaps.put(hotspots.get(i).name,
                        Glide.with(this)
                                .load(hotspots.get(i).imageUrl)
                                .asBitmap()
                                .diskCacheStrategy(DiskCacheStrategy.SOURCE)
                                .into(Constants.WEAR_IMAGE_SIZE, Constants.WEAR_IMAGE_SIZE)
                                .get());
        ...
        // The intent to trigger when the notification is tapped
        PendingIntent pendingIntent = PendingIntent.getActivity(this, 0,
                DetailHotSpotActivity.getLaunchIntent(this, hotspot.name),
                PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this)
                .setStyle(new NotificationCompat.BigPictureStyle()
                                .bigPicture(bitmaps.get(hotspot.name))
                                .setBigContentTitle(hotspot.name)
                                .setSummaryText(getString(R.string.nearby_hotspot))
                )
                .setLocalOnly(microApp)
                .setContentTitle(hotspot.name)
                .setContentText(getString(R.string.nearby_hotspot))
                .setSmallIcon(R.drawable.ic_stat_maps_pin_drop)
                .setSound(soundUri) // von flo um einen sound zu erhalten
                .setContentIntent(pendingIntent)
                .setDeleteIntent(deletePendingIntent)
        ...
        // Trigger the notification
        NotificationManagerCompat.from(this).notify(
                Constants.HOTSPOT_NOTIFICATION_ID, builder.build());
    }
